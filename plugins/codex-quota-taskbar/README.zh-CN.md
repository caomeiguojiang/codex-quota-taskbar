...
![任务栏内垂直居中的额度浮层](assets/screenshot.png)

![浮层右键菜单](assets/menu.png)

## 任务状态同步支持

支持同步 Codex Desktop 的对话执行状态。状态图标位于浮层最左侧，不额外占用文字区域。

![Idle 到 Running 到 Complete 的状态同步动图](assets/task-status-sync-real.gif)
...
![从左侧导航栏进入插件页](assets/desktop-sidebar-plugins.png)
...
```text
companion\bin\CodexQuotaTaskbar.exe
